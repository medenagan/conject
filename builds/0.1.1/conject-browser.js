/* conject 0.1.1 | (c) Fabio Mereu | MIT license | https://github.com/medenagan/conject | 2019-01-10T22:14+01:00 */
...
} : (typeof performance === "undefined" ? "undefined" : _typeof(performance)) === "object" && typeof performance.now === "function" ? performance.now.bind(performance) : Date.now; // Helper to handle multiple callbacks listening to an event
...
function (_StaticCondition) {
  ...
  function TrueCondition() {
    ...
    if (_.has(TrueCondition)) return _possibleConstructorReturn(_this3, _.get(TrueCondition));
    _this3 = _possibleConstructorReturn(this, _getPrototypeOf(TrueCondition).call(this, true));

    _.set(TrueCondition, _assertThisInitialized(_assertThisInitialized(_this3)));
...
function (_StaticCondition2) {
  ...
  function FalseCondition() {
    ...
    if (_.has(FalseCondition)) return _possibleConstructorReturn(_this4, _.get(FalseCondition));
    _this4 = _possibleConstructorReturn(this, _getPrototypeOf(FalseCondition).call(this, false));

    _.set(FalseCondition, _assertThisInitialized(_assertThisInitialized(_this4)));
...
function (_Condition4) {
  ...
  function FunctionalCondition(fn) {
    ...
    _this5 = _possibleConstructorReturn(this, _getPrototypeOf(FunctionalCondition).call(this, {
      fn: fn
    }));
    if (typeof fn !== "function") throw new Error(_this5.constructor.name + " must be constructed from a function");
    return _this5;
  }

  _createClass(FunctionalCondition, [{
    key: "test",
    value: function test(initialValue, scope) {
      var _$get8 = _.get(this),
          fn = _$get8.fn;

      return new Evaluation(function (resolve, reject) {
        try {
          var result = fn.call(null, initialValue);
...
function (_Condition5) {
  ...
  function PromiseCondition(promise) {
    ...
    _this6 = _possibleConstructorReturn(this, _getPrototypeOf(PromiseCondition).call(this, {
      promise: promise
    }));
    if (!(promise instanceof Promise)) throw new Error(_this6.constructor.name + " must be constructed from a Promise");
    return _this6;
  }
  ...
  _createClass(PromiseCondition, [{
    ...
    value: function test() {
      ...
      return new Evaluation(function (resolve, reject) {
        return _.get(_this7).promise.then(resolve, reject);
      });
    }
  }, {
    key: "toString",
...
function (_Condition6) {
  ...
  function ConditionList() {
    ...
    _this8 = _possibleConstructorReturn(this, _getPrototypeOf(ConditionList).call(this, {
      conditions: conditions
    }));
    if (conditions.some(function (condition) {
      return !(condition instanceof Condition);
    })) throw new Error(_this8.constructor.name + " must be constructed from Condition objects only");
    if (conditions.length < 2) throw new Error(_this8.constructor.name + " must be constructed from at least two Condition objects");
    return _this8;
  }
...
var Sequential = function Sequential(operator) {
  return (
    ...
    function (_ConditionList) {
      ...
      _createClass(_class, [{
        key: "test",
        value: function test(initialValue) {
          ...
          return new Evaluation(resolver, function () {
            aborted = true;
        ...
        value: function test() {
          ...
          var resolver = function resolver(resolve, reject) {
            ...
            subEvaluations = conditions.map(function (condition) {
              var subEvaluation = condition.test.apply(condition, params);
...
function (_Condition7) {
  ...
  function UnaryProxyCondition(source) {
    ...
    _this9 = _possibleConstructorReturn(this, _getPrototypeOf(UnaryProxyCondition).call(this, Object.assign({}, members, {
      source: source
    })));
    if (!(source instanceof Condition)) throw new Error(_this9.constructor.name + " must be constructed from a Condition object");
    return _this9;
  }
...
function (_UnaryProxyCondition) {
  ...
  function DelayedCondition(source) {
    ...
    _this10 = _possibleConstructorReturn(this, _getPrototypeOf(DelayedCondition).call(this, source));
    delay = +delay;
    if (Number.isNaN(delay)) throw new Error(_this10.constructor.name + "(..., delay) requires a numeric value or it can be omitted");
    if (delay < 0) delay = 0;
    if (delay > MAX_TIMEOUT) return _possibleConstructorReturn(_this10, new EternalCondition());
    _.get(_assertThisInitialized(_assertThisInitialized(_this10))).delay = delay;
    return _this10;
  }
...
function (_UnaryProxyCondition2) {
  ...
  function DurableCondition(source) {
    ...
    _this11 = _possibleConstructorReturn(this, _getPrototypeOf(DurableCondition).call(this, source));
    duration = +duration;
    ...
    if (!duration) return _possibleConstructorReturn(_this11, source);
...
function (_UnaryProxyCondition3) {
  ...
  function TimeoutCondition(source) {
    ...
    _this12 = _possibleConstructorReturn(this, _getPrototypeOf(TimeoutCondition).call(this, source));
    duration = +duration;
    if (Number.isNaN(duration)) throw new Error(_this12.constructor.name + " expects a numeric duration or it can be omitted");
    if (duration > MAX_TIMEOUT) return _possibleConstructorReturn(_this12, source);
    if (duration <= 0) return _possibleConstructorReturn(_this12, FALSE);
    _.get(_assertThisInitialized(_assertThisInitialized(_this12))).duration = duration;
    return _this12;
  }
...
function (_UnaryProxyCondition4) {
  ...
  function CycleCondition(source) {
    ...
    _this13 = _possibleConstructorReturn(this, _getPrototypeOf(CycleCondition).call(this, source));
    attempts = +attempts;
    if (Number.isNaN(attempts)) throw new Error(_this13.constructor.name + " requires an integer for attempts or it can be omitted");
    if (attempts < 1) return _possibleConstructorReturn(_this13, FALSE);
    attempts = Math.floor(attempts);
    _.get(_assertThisInitialized(_assertThisInitialized(_this13))).attempts = attempts;
    return _this13;
  }
...
function (_UnaryProxyCondition5) {
  ...
  function NegativeCondition(source) {
    ...
    _this14 = _possibleConstructorReturn(this, _getPrototypeOf(NegativeCondition).call(this, source)); // (!(! x)) => Bool(x)

    if (source instanceof NegativeCondition) return _possibleConstructorReturn(_this14, new BooleanCondition(source.source)); // (!(Bool(x)) => (!x)

    if (source instanceof BooleanCondition) return _possibleConstructorReturn(_this14, new NegativeCondition(source.source));
    return _this14;
  }
...
function (_UnaryProxyCondition6) {
  ...
  function BooleanCondition(source) {
    ...
    _this15 = _possibleConstructorReturn(this, _getPrototypeOf(BooleanCondition).call(this, source)); // Boolean(Boolean(x)) = Boolean(x);  Boolean(!(x)) = !(x)

    if (source instanceof BooleanCondition || source instanceof NegativeCondition) return _possibleConstructorReturn(_this15, source);
    return _this15;
  }
...
function (_UnaryProxyCondition7) {
  ...
  function EventCondition(source, onResolve, onReject) {
    ...
    _this16 = _possibleConstructorReturn(this, _getPrototypeOf(EventCondition).call(this, source));
    _.get(_assertThisInitialized(_assertThisInitialized(_this16))).onResolve = typeof onResolve === "function" ? onResolve : function (_) {
      return undefined;
    };
    _.get(_assertThisInitialized(_assertThisInitialized(_this16))).onReject = typeof onReject === "function" ? onReject : function (_) {
      return undefined;
    };
    return _this16;
  }
...
function (_EventCondition) {
  ...
  function TrueEventCondition(source, handler) {
    ...
    _this17 = _possibleConstructorReturn(this, _getPrototypeOf(TrueEventCondition).call(this, source, onResolve));
...
function (_EventCondition2) {
  ...
  function FalseEventCondition(source, handler) {
    ...
    _this18 = _possibleConstructorReturn(this, _getPrototypeOf(FalseEventCondition).call(this, source, onResolve));
...
function (_EventCondition3) {
  ...
  function ErrorEventCondition(source, handler) {
    ...
    _this19 = _possibleConstructorReturn(this, _getPrototypeOf(ErrorEventCondition).call(this, source, undefined, handler));
    if (typeof handler !== "function") throw new Error(_this19.constructor.name + " needs a function as handler");
    return _this19;
  }

  return ErrorEventCondition;
}(EventCondition); // Colors for console
...
var CHAINABLE_PROTOTYPE = {
  ...
  run: function run(initialValue) {
    ...
    var evaluation = this.condition.test(initialValue);
    evaluation.on();
    return evaluation;
  },
...
var isChainable = function isChainable(what) {
...
= function Chainable(source) {
  ...
  var chain = isChainable(source) ? _.get(source).chain.slice() : [];
...
[{
  ...
  starter: true,
  compile: function compile(root) {
    var error = new ErrorCondition(arguments.length <= 1 ? undefined : arguments[1]);
...
var fromValue = function fromValue(value) {
  return new StaticCondition(value);
};

var fromFunction = function fromFunction(fn) {
  return new FunctionalCondition(fn);
};

var fromPromise = function fromPromise(promise) {
  return new PromiseCondition(promise);
};
