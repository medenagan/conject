  ...
  class DisposableMegaphone {
    ...
    speak(...params) {
      ...
      if (self.used)
        throw new Error(this.constructor.name + ".speak() must be called only once."
                        + " Cannot accept " + String(params)
                        + " after " + String(self.params));
    ...
    listen(callback) {
      ...
      if (typeof callback !== "function")
        throw new Error(this.constructor.name + ".listen requires a function as callback");

      const {used, params, callbacks} = _.get(this);
  ...
  */
  ...
  class Evaluation {
    ...
    constructor (resolver, aborter = () => undefined) {
      ...
      if (typeof resolver !== "function")
        throw new Error(this.constructor.name + " resolver " + String(resolver) + " is not a function");

      if (typeof aborter !== "function")
        throw new Error(this.constructor.name + " aborter " + String(aborter) + " is not a function");

      _.set(this, {resolver, aborter, done: false, status: EVALUATION_PENDING, queue: new DisposableMegaphone()});
    }

    abort(reason = "aborted") {

      const {status, aborter} = _.get(this);
    ...
    get async() {
      ...
      const {status, async} = _.get(this);

      if (status === EVALUATION_PENDING)
        throw new Error(".async does not exist for [" + status + "] status");
    ...
    get value() {
      ...
      const {status, value} = _.get(this);

      if (status !== EVALUATION_RESOLVED)
        throw new Error(".value does not exist for [" + status + "] status");
    ...
    get reason() {
      ...
      const {status, reason} = _.get(this);

      if (status !== EVALUATION_REJECTED)
        throw new Error(".reason does not exist for [" + status + "] status");
  ...
  class Condition {
    ...
    test() {
      return Evaluation.reject(
        new Error(this.constructor.name + ".test() must be implemented on a derived class")
      );
    }
  ...
  class FunctionalCondition extends Condition {
    ...
    constructor (fn) {
      ...
      if (typeof fn !== "function")
        throw new Error(this.constructor.name + " must be constructed from a function");
    }
  ...
  class PromiseCondition extends Condition {
    ...
    constructor (promise) {
      ...
      if (! (promise instanceof Promise))
        throw new Error(this.constructor.name + " must be constructed from a Promise");
    }
    ...
    test () {
      return new Evaluation(
        (resolve, reject) => _.get(this).promise.then(resolve, reject)
      );
    }
  ...
  class ConditionList extends Condition {
    ...
    constructor (...conditions) {
      ...
      if (conditions.some(condition => ! (condition instanceof Condition)))
        throw new Error(this.constructor.name + " must be constructed from Condition objects only");

      if (conditions.length < 2)
        throw new Error(this.constructor.name + " must be constructed from at least two Condition objects");
    }
  ...
  class UnaryProxyCondition extends Condition {
    ...
    constructor (source, members = {}) {
      super(Object.assign({}, members, {source}));

      if (! (source instanceof Condition))
        throw new Error(this.constructor.name + " must be constructed from a Condition object");
    }
  ...
  class DelayedCondition extends UnaryProxyCondition {
    ...
    constructor (source, delay = 0) {
     ...
     if (Number.isNaN(delay))
        throw new Error(this.constructor.name
          + "(..., delay) requires a numeric value or it can be omitted");
    ...
    test(...params) {
      ...
      const {source, delay} = _.get(this);
      let handle, subEvaluation;

      return new Evaluation(

        (resolve, reject) => (handle = setTimeout(
          () => {
            handle = null;
  ...
  class DurableCondition extends UnaryProxyCondition {
    ...
    constructor (source, duration = 0) {
      ...
      if (Number.isNaN(duration))
        throw new Error(this.constructor.name + " expects a numeric duration or it can be omitted");
    ...
    test(...params) {
      ...
      const {source, duration} = _.get(this);

      const t0 = now();

      let aborted; let subEvaluation;
  ...
  class CycleCondition extends UnaryProxyCondition {
    ...
    constructor (source, attempts = 1) {
      ...
      if (Number.isNaN(attempts))
        throw new Error(this.constructor.name
          + " requires an integer for attempts or it can be omitted");
    ...
    test (...params) {
      ...
      const {source, attempts} = _.get(this);
  ...
  class NegativeCondition extends UnaryProxyCondition {
    ...
    constructor (source) {
      ...
      // (!(! x)) => Bool(x)
      if (source instanceof NegativeCondition)
        return new BooleanCondition(source.source);

      // (!(Bool(x)) => (!x)
      if (source instanceof BooleanCondition)
        return new NegativeCondition(source.source);
    }

    test (...params) {

      const {source} = _.get(this);
  ...
  class BooleanCondition extends UnaryProxyCondition {
    ...
    constructor (source) {
      ...
      if (source instanceof BooleanCondition || source instanceof NegativeCondition)
        return source;
    }
  ...
  class EventCondition extends UnaryProxyCondition {
    ...
    test(...params) {
      ...
      const {source, onResolve, onReject} = _.get(this);
  ...
  class TrueEventCondition extends EventCondition {
    ...
    constructor (source, handler) {
      ...
      if (typeof handler !== "function")
        throw new Error(this.constructor.name + " needs a function as handler");
  ...
  const isChainable = (what) => (what = _.get(what))
                             && (typeof what === "object")
  ...
  */
  ...
  class Chainable /* extends Function */ {
    ...
    constructor (source) {
      ...
      const chain = isChainable(source)
...
//      constructor: SequentialAnd, // .if(c0).if(c1) === if(c0).and(c1)
    ...
    [
      {starter: true, name:"and", constructor:SequentialAnd},
      {starter: true, name:"or", constructor:SequentialOr},
      {starter: true, name:"xor", constructor:SequentialXor},
      {starter: true, name:"nor", constructor:SequentialNor},
      {starter: true, name:"nand", constructor:SequentialNand},
      {starter: true, name:"xnor", constructor:SequentialXnor},
      {starter: true, name:"anda", constructor:ParallelAnd},
      {starter: true, name:"ora", constructor:ParallelOr},
      {starter: true, name:"xora", constructor:ParallelXor},
      {starter: true, name:"nora", constructor:ParallelNor},
      {starter: true, name:"nanda", constructor:ParallelNand},
      {starter: true, name:"xnora", constructor:ParallelXnor},
    ]
    ...
    [
      ...
      {name: "bool", constructor: BooleanCondition}
    ]
    .map(method => Object.assign(method, {
      compile: (root, ...params) => {
        if (params.length)
    ...
    [
      ...
      {name: "out", constructor: TimeoutCondition},
      {name: "during", constructor: DurableCondition},
      {name: "atmost", constructor: CycleCondition}
    ]
    .map(method => Object.assign(method, {
      compile: (root, ...params) => {
    ...
    ... /* .onevent(fn | arg0) */
    [
      {name: "onTrue", constructor: TrueEventCondition, defaultHandler: console.log},
      {name: "onFalse", constructor: FalseEventCondition, defaultHandler: console.warn},
      {name: "onError", constructor: ErrorEventCondition, defaultHandler: console.error},
    ]
    .map(method => Object.assign(method, {
      compile: (root, ...params) => {
