...
const now = typeof process === "object" && typeof process.hrtime === "function" ? (a, b) => ([a, b] = process.hrtime(), a * 1e3 + b * 1e-6) : typeof performance === "object" && typeof performance.now === "function" ? performance.now.bind(performance) : Date.now; // Helper to handle multiple callbacks listening to an event
...
matryoshkingMethods.forEach(method => {
  ...
  CHAINABLE_PROTOTYPE[name] = function (...params) {
    ...
    const {
      chain
    } = _.get(clone); // root is the most external Condition
